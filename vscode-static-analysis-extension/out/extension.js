...
async function activate(context) {
    ...
    // 	})
    // );
    context.subscriptions.push(vscode.commands.registerCommand('static-analysis.navigateToFunction', (f) => {
        provider.showFunction(f.id);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('static-analysis.references', (f) => {
        provider.showReferences(f);
    }));
    let last_manual_mapped_func = null;
    context.subscriptions.push(vscode.commands.registerCommand('static-analysis.manuallyMapFunctionRelationship', (f) => {
    ...
    new codelens_1.StaticAnalysisCodeLensProvider({ pattern: '**/*' }, provider.functionDefinitions.filter(f => { return !f.is_inherited; }), provider.getAuditCommentsLineOffset, provider.getFileLines, provider.sourceFilesCache)));
...
class StaticAnalysisViewProvider {
    constructor(_extensionUri) {
        this._extensionUri = _extensionUri;
        this.DATA_PATH = path.join(".vscode", "ext-static-analysis");
        this.helpHTML = '';
        this.lastManuallyMappedFunction = null;
        this.scopeSummaries = [];
        this.functionDefinitions = [];
        this.functionDefinitionsMap = new Map();
        this.functionManualRelationship = []; // not making a hashmap because this list should never get too large
        this.defaultFileLineColumnOffset = {}; // used to find column when opening function, primarily when id does not contain column number
        this.scopeDefinitions = {}; // can also be used to enumerate scopes themselves
        this.func_pair_edge_colors = {};
        this.decoratorUnicode = "";
        this.functionSortOption = "Alpha. + Line #";
        this.searchTemplates = [];
        this.funcStateVarReadWrittenMapping = {};
        this.scopeGraphs = {};
        this.inheritanceGraph = { nodes: [], edges: [] };
        this.hasInheritedFunctions = false;
        this.relatedCallstacksHash = {};
        ...
        this.settings = { excludedRelatedCallstacks: ["slitherConstructorConstantVariables", "constructor", "initialize", "initializer", "init", "__init__", "run", "main", "__main__"], manualFunctionRelationshipPath: "", showAllFunctions: false };
        this.callstacksGraphCache = {}; // should use a data structure to separate: groups | nodes | edges ?
        this.sourceFilesCache = {};
        this.currentFilteredFunctionState = { regexPattern: "", excludeRegexPattern: "", filteredFunctionIds: [], hideReviewedState: "Hide Reviewed Except In Scope" }; // sets default
    }
    ...
    async appendAndCreateFolder(folderName) {
        const workspaceFolderPath = this.getFirstWorkspaceFolderPath();
    ...
    isIconChar(c) {
        ...
        if (!code)
            return false;
        return ((code >= 0x2600 && code <= 0x26FF) || // Miscellaneous Symbols
    ...
    getRelatedCallstacks(f) {
        if (this.relatedCallstacksHash[f.id]) {
            return this.relatedCallstacksHash[f.id];
        }
        let known_callstacks = [];
        if (f.entrypoint_callstacks)
            known_callstacks = known_callstacks.concat(f.entrypoint_callstacks);
        if (f.exit_callstacks)
            known_callstacks = known_callstacks.concat(f.exit_callstacks);
        if (f.other_callstacks)
            known_callstacks = known_callstacks.concat(f.other_callstacks);
        let related_callstacks_ixs = [];
    ...
    getScopeGraph(scope_id, return_scopes_only = false, include_inherited_functions = true, include_related_function_callstacks = false) {
        ...
        if (!this.scopeGraphs[cache__key]) {
            ...
            let functions_to_chart = scope_id.startsWith('all~') ? this.functionDefinitions
            ...
            let mergeCallstacksIntoGraphParams = {
                graph: { nodes: [], edges: [] },
            ...
            if (!return_scopes_only) {
                ...
                let seen_parent_ids = graph.nodes.filter(n => { return 'isParent' in n.data && n.data.isParent === true; }).map(n => { return n.data.id; });
    ...
    mergeCallstacksIntoGraph({ graph, seen_nodes = [], seen_edges = [], callstacksGraphParams, override_color = "" }) {
    ...
    async loadFunctionsAndScopeInfo() {
        // export async function readResults(print : boolean = false) : Promise<boolean> {
        // Verify there is a workspace folder open to run analysis on.
        if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length == 0) {
            return false;
        ...
        for (let i = 0; i < vscode.workspace.workspaceFolders.length; i++) {
            ...
            if (fs.existsSync(manualFunctionRelationshipPath)) {
                ...
                if (!this.functionManualRelationship)
                    return false;
            }
            // If the file exists, we read its contents into memory.
            const resultsPath = path.join(workspacePath, this.DATA_PATH, 'functions_html.json');
            ...
            if (fs.existsSync(funcStateVarReadWrittenMappingPath)) {
                ...
                if (!this.funcStateVarReadWrittenMapping)
                    return false;
            }
            // load callstacks
            const callstacksPath = path.join(workspacePath, this.DATA_PATH, 'callstacks.json');
    ...
    getFileSource(filepath, startLine = null, endLine = null, filter_audit_comments = true) {
    ...
    getAuditCommentsLineOffset(filepath, originalLineNum) {
        ...
        function getOffset(startLine, newStartLine, lines) {
    ...
    updateGraphRelatedFunctionHTMLLinks(nodes, funcs_seen = []) {
        ...
        for (let node of nodes) {
            if (!('title' in node.data))
                continue;
            let lookup_func = this.functionDefinitionsMap.get(node.data.id);
            if (lookup_func) {
    ...
    getManuallyMappedRelationships(func_id, { return_scopes_only = false, seen_relationships = [], root_scope_id = "", append_related_function_html = true, directional_edges_only = undefined }) {
        let nodes = [];
        let edges = [];
        ...
        for (let relationship of manually_mapped_relationships) {
            ...
            let callee_lookup_func = this.functionDefinitionsMap.get(relationship.callee_id);
            let caller_lookup_func = this.functionDefinitionsMap.get(relationship.caller_id);
    ...
    getCallstacksGraph({ callstacks, cache__key = "", return_scopes_only = false, seen_relationships = [], root_scope_id = "", append_related_function_html = true, directional_edges_only = undefined }) {
        ...
        let root_scope = this.scopeSummaries.find(s => { return s.id === root_scope_id; });
        let scopes_seen = [];
        let funcs_seen = [];
        let nodes = [];
        let edges = [];
        callstackLoop: for (let i of callstacks) {
            ...
            let callstack = this.callstacks[i].map(ele => { return this.getFunctionId(ele); });
            let getCallstackSubset = (callstack) => {
                let startIndex = 0, endIndex = callstack.length;
                // Loop through callstack only once
                callstack.some((func_id, index) => {
                    const f = this.functionDefinitionsMap.get(func_id);
                    if (f && f.hide_incoming_callstacks)
                        startIndex = index;
            ...
            functionLoop: for (let [j, func_id] of callstack.entries()) {
                ...
                let lookup_func = this.functionDefinitionsMap.get(func_id);
                if (lookup_func) {
                    if (return_scopes_only) {
                ...
                let manualMappedRelationshipsGraph = this.getManuallyMappedRelationships(func_id, params);
    ...
    getFunctionDescriptiveStr(f, hyperlinedHTML = false, include_manual_relationship_link = false, function_params_newline = false) {
        if (hyperlinedHTML) {
            // graph
    ...
    filterFunctionDefinitions(functions, regexPattern = "", excludeRegexPattern = "") {
    ...
    showScope(scope_id, checkbox_ids_to_check) {
        if (this._view) {
    ...
    showReferences(f) {
        ...
        vscode.commands.executeCommand('editor.action.peekLocations', vscode.Uri.file(start_filepath), new vscode.Position(f.startLine + this.getAuditCommentsLineOffset(f.filepath, f.startLine), 0), reference_ranges);
    }
    showFunction(f_id, navigated_from_history = false, mode = "Combined", include_related_callstacks = false) {
        if (this._view) {
            this._view.show?.(true); // `show` is not implemented in 1.49 but is for 1.50 insiders
            let func = this.functionDefinitions.find(f => { return f.id == f_id && !f.is_inherited; });
            if (!func)
                return;
            let getCallstacksGraphParams = {
                callstacks: [],
                cache__key: "",
            ...
            let related_callstack_indexes = this.getRelatedCallstacks(func);
            ...
            let exclude_list = ['interface', 'test', 'mock'];
            // exact matches
    ...
    showFilteredScopesRegex(regexPattern, excludeRegexPattern) {
        const max_filesize = 6;
        excludeRegexPattern = excludeRegexPattern.trim();
        if (this._view) {
            this._view.show?.(true); // `show` is not implemented in 1.49 but is for 1.50 insiders
    ...
    showFilteredFunctionsRegex(regexPattern, excludeRegexPattern) {
        if (this._view) {
            this._view.show?.(true); // `show` is not implemented in 1.49 but is for 1.50 insiders
            // get functions
    ...
    joinGraphByFunction(graph_nodes, graph_edges, f_id, direction) {
        let graph = { nodes: graph_nodes, edges: graph_edges };
    ...
    manuallyMapFunctionRelationship(f_id) {
    ...
    async resolveWebviewView(webviewView, resolveContext, _token) {
        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            ...
            if (message.command === "mark_function_reviewed") {
                ...
                if (func) {
                    ...
                    // update graph cache
                    Object.keys(this.callstacksGraphCache).forEach((key) => {
                        const graph = this.callstacksGraphCache[key];
    ...
    _getHtmlForWebview(webview) {
        ...
        const enableUnsafeEval = vscode.workspace.getConfiguration('static-analysis').get('enableUnsafeEval');
        return `<!DOCTYPE html>
			<html lang="en">
            ...
            return `<option data-include="${include}" data-exclude="${exclude}" data-highlight="${highlight}">${t.name || ""}: ${t.include} | ${t.exclude}</option>`;
        }).join("")}
...
StaticAnalysisViewProvider.viewType = 'staticanalysis.view';
function getNonce() {
    let text = '';

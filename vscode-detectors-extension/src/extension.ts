...
export async function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("slither.analyze", async () => {
        if(!analysisRunning) {
            ...
            vscode.window.withProgress(progressOptions, async (progress, token) => {
                ...
                await detectorFilterTreeProvider.setDetectorFilterCounts(slitherExplorerTreeProvider.rootNode)
                analysisRunning = false;
            });
        }
    }));

    context.subscriptions.push(vscode.commands.registerCommand("slither.saveResults", async () => {
        Logger.log("save results clicked")
        await slither.writeResults()
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand("slither.toggleFalsePositive", async (node : explorer.CheckResultNode) => {
        await slither.toggleFalsePositive(node.result.id);
        await slitherExplorerTreeProvider.refreshExplorer(false);
    }));

...
export async function analyze(print : boolean = true) : Promise<boolean> {
    ...
    Logger.log(`Starting analysis @ ${new Date()}`);
    ...
    for (let i = 0; i < vscode.workspace.workspaceFolders.length; i++) {
        ...
        let { output, error } = await exec_slither(`${config.userConfiguration.slitherArgs || ''} ${config.userConfiguration.slitherTarget || '.'} --json -`, false, workspaceFolder);
...
${error!.toString()}`);
    ...
    Logger.log(`Time to analyze: ${minutes}:${seconds} min:sec`);
...
export async function loadDetectors() {
    ...
    // Verify there is a workspace folder open to run analysis on.
    if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length == 0) {
        Logger.error('There are no open workspace folders to load detectors for.');
        return;
    }
    ...
    for (let workspaceFolder of vscode.workspace.workspaceFolders) {
        ...
        if (fs.existsSync(detectorDetailsPath)) {
            try {
                ...
                Logger.log(`Failed to read detector details from ${detectorDetailsPath}: ${e}`)
            }
        } else {
            ...
            try {
                ...
                // Print our error and return a null array.
                Logger.error(`Failed to load slither detector details: ${e}`);
            }
        }
...
export async function toggleFalsePositive(id: string) {
    // export async function readResults(print : boolean = false) : Promise<boolean> {
    // Verify there is a workspace folder open to run analysis on.
    if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length == 0) {
    ...
    for (let i = 0; i < vscode.workspace.workspaceFolders.length; i++) {
        ...
        if(fs.existsSync(resultsPath)) {
            ...
            let workspaceResults : SlitherResult[] | undefined = results.get(workspacePath);
            ...
            if (targetObject) {
                ...
                Logger.log(`False Positive : Current value (${targetObject.falsePositive}) -> Setting to: (${!targetObject.falsePositive})`)
...
export async function writeResults() : Promise<boolean> {
    Logger.log("Saving...")
    ...
    for (let i = 0; i < vscode.workspace.workspaceFolders.length; i++) {
        ...
        if(fs.existsSync(resultsPath)) {
            ...
            if (!workspaceResults) return false;
...
function setDetectors(results: SlitherResult[] ) {
    let _detectors: any[] = []
...
export async function printResult(item : SlitherResult, filterDetectors : boolean = true) {
    ...
    Logger.log(result_with_fullpaths);
...
async function exec_slither(args : string[] | string, logError : boolean = true, workingDirectory : string | undefined = undefined) : Promise<{output : string, error : string}> { 
    ...
    Logger.log(SlitherCmd)
    let { stdout } = await cmd(SlitherCmd, { cwd : workingDirectory, maxBuffer: maxBufferSize}).catch((e: any) => error = e);

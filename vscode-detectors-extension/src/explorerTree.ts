...
export class SlitherExplorer implements vscode.TreeDataProvider<ExplorerNode> {
    ...
    public bySeverityNode : ExplorerNode = new ExplorerNode("By Severity");
    ...
    public rootNode : ExplorerNode = this.bySeverityNode;
    ...
    constructor(private context: vscode.ExtensionContext) {
        ...
        this.setIconBySeverity(criticalSeverityNode, <string>criticalSeverityNode.label);
    ...
    public async printDetailedDescription(node : CheckResultNode) {
        ...
        if(!detector) {
            Logger.error(`Detector "${node.result.check}" could not be resolved. Detailed output may be impacted.`);
            // return;
        }
        ...
        Logger.log(
`Type: ${detector?.title || node.result?.title || ''} (${node.result.check})
Impact: ${node.result?.impact || detector?.impact || ''}
Confidence: ${node.result?.confidence || detector?.confidence || ''}
Finding:`);
        ...
        Logger.log(
`Recommendation: ${detector?.recommendation || node.result?.reccomendation || ''}

Reference: ${detector?.wiki_url || node.result?.wiki_url || ''}`
);
